...
##  Selcts uniq frequencies, for those frequencies selects uniq accelerations, and for each set of uniq frequency and acceleration, select max power.
#  This step is to remove the duplicate detections in the HSUM tree.
    hms=8
    uniq_frq=[]
    uniq_accl=[]
    uniq_pwr=[]
...
def flag_within_harmonic(freq,acc,pwr,hm):
    ...
    if(hm != 1):
        ...
        for fseed in seed_f:
            ...
            fl=np.round(hm*(f0-1.0))
            fu=np.round(hm*(f0+1.0))
            ...
            for aseed in seed_a1:
                ...
                al=np.round(hm*(a0-2.0))
                au=np.round(hm*(a0+2.0))
...
if(len(sys.argv) < 2):
    print("python FDAS_sifting_preliminary.py <logfile name>")
    sys.exit()
logfile = sys.argv[1]
