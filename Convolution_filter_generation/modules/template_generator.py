...
def Compute_template(drift):
    ...
    template1 = (
        1.0
        / np.sqrt(2.0 * drift)
        * np.exp(1j * np.pi * (q - drift / 2.0) ** 2.0 / drift)
    )
...
class filter_padding_and_fft:
    ...
    def padding(self):
        ...
        padded_filter = np.roll(
            padded_filter, int(self.max_fil_len / 2.0 - self.fil_len / 2.0)
        )
...
class final_filter_write:
    def __init__(
        self, filter_fft, file_base
    ):  ###  filter_fft is fft of the filter and file_base is the pointer to opened file
        self.filter = filter_fft
        self.file = file_base
