...
Feature: SPCCL metadata in the envelope
  ...
  Scenario: The routing identifiers are carried alongside the SPCCL scalars
    ...
    And the message key is "<scheduling_block_id>:<beam_id>"
  ...
  Scenario: An MJD packed as float32 is rejected
    ...
    Given the candidate is serialised with "spccl.mjd" packed as float32
  ...
  Scenario Outline: A missing SPCCL field is rejected
    ...
    Given the candidate is serialised without "spccl.<field>"
    Then the consumer rejects it with ContractViolationError
  ...
  Scenario: An SPCCL record that is not a map is rejected
    Given the candidate is serialised with "spccl" as a string
    Then the consumer rejects it with ContractViolationError
